...
//! [`epubsana::VERSION`] and the wasm binding all print the exact source they
//! were built from (veripublica conventions v0.4, CLI.md §3.1).
//!
//! Emits `EPUBSANA_BUILD` as `+<short-hash>[.dirty]`, or **empty** when there is
...
fn main() {
    ...
    println!("cargo:rustc-env=EPUBSANA_BUILD={build}");
